...
codigo = codigo.split(/([0-9]+|<=|>=|!=|[|][|]|[&][&]|<|>|switch|else|if|case|break|default|do|for|while|Fin|print|input|int|float|char|string|bool|palabra|caracter|num|id|true|false|.)/).filter(Boolean)
...
function Analizar() {
    ...
    do {
        ...
        if (entrada == '$' && TABLA[fila][columna] == 'acc') { //se acepta{
        ...
        if (TABLA[fila][columna].charAt(0) == 's') {
            ...
            if (tipos_dato.indexOf(codigo[i]) != -1) {
                ...
                // etiuno = 'etiq' + (++cont_etiq)
                // pila_etiquetas.push(etiuno + ': ;\n')
                // etiqueta_else.push('Var' + cont_var)
                ...
                //pila_switch.push('Var' + cont_var)
                pila_etiquetas.push('goto ' + pila_etiquetas[pila_etiquetas.length - 1]  )


                //codigo_obj += 'Var' + cont_var
                ban_default = false
            } else if (ban_case) {
                ArmarPosfija(codigo[i])
                if (codigo[i] == '{') {
                    tipo1 = posfija_tipos.pop()
                    //ASDeclararVariableAuxiliar('Var' + (++cont_var), tipo1)
                    //codigo_obj += tipo1+' Var' + (cont_var)+' ;\n';
                    ...
                    codigo_obj += 'if( !( ' + pila_switch[pila_switch.length-1] + ' == ' + posfija.pop() + ' ) ) \ngoto ' + etiuno + ' ;\n'
                    //}
                ...
                if (codigo[i] == '{') {
                    ...
                    pila_switch.push('Var' + cont_var)
                    pila_etiquetas.push('etiq' + (++cont_etiq)+':;\n')
                ...
                //codigo_obj +='goto etiq'+(++cont_etiq)+';\n' +pila_etiquetas.pop() + ':\n'+etiqueta_else+': \n'
                codigo_obj += pila_etiquetas.pop()
            } else if (entrada == '=') {
                ...
                if (ban_asignacion) {
                    ...
                    if (tabla_de_variables["Var" + cont_var].Tipo == tabla_de_variables[id_asignar].Tipo) {
                        cod_obj_id_asigna += 'Var' + cont_var;
                        if (posfija_tipos.length != 0) posfija_tipos.pop()
                        if (posfija_tipos_booleanos.length != 0) posfija_tipos_booleanos.pop()
                    } else {
                        //hay error de tipos
                ...
                if (ban_declaracion) codigo_obj += codigo[i] + ' '
            } else {}
...
function A_L(ent) {
    ...
    if (palab_clave.indexOf(ent) != -1) {
        ...
        if (terminador /*;*/ == ent) {
            tipo_dato_actual = ''
            // console.log(pila_tipos_dato)
            // pila_tipos_dato.length = []

            //// primero debe asignar a la variable y luego 
            //// Valor= al valor obtenido en la pila posfija 
            //// console.log("************************"+pila_valores_dato[0])
            //if (ban_asignacion) {
            //// console.log("************************"+pila_valores_dato[0])
            //console.log(id_asignar)
            ////analizar la expresion posfija en ambas pilas ( valores y tipos, para confirmar que esta todo bien )
            //Analizar_Posfija()
            //// tabla_de_variables[id_asignar].Valor = pila_valores_dato[0]
            //id_asignar = ''
            //}
            ...
            return ent
        } else if (RegEx_Variable.test(ent) && !RegEx_Numero_Int.test(ent) && boelanos.indexOf(ent) == -1) {
            if (tipo_dato_actual != '') {
        ...
        if (operacion.indexOf(posfija[posfija.length - 1]) != -1) {
            ...
            if ((tipo1 == "int" || tipo1 == "float") && (tipo2 == "int" || tipo2 == "float")) {
                if (tipo1 == tipo2) {
                    ...
                    pilaS.push(' Var' + (++cont_var))
                    ASDeclararVariableAuxiliar(pilaS[pilaS.length - 1].trim(), tipo1)
                    codigo_obj += tipo1 + pilaS[pilaS.length - 1] + ' = ' + var2 + ' ' + posfija.pop() + ' ' + var1 + ';\n'
                } else {
                    pilaS.push("float")
            ...
            if ((tipo1 == "int" || tipo1 == "float") && (tipo2 == "int" || tipo2 == "float")) {
                ...
                pilaS_booleanos.push(' Var' + (++cont_var))
                ASDeclararVariableAuxiliar(pilaS_booleanos[pilaS_booleanos.length - 1].trim(), 'bool')
                codigo_obj += "bool" + pilaS_booleanos[pilaS_booleanos.length - 1] + ' = ' + var2 + ' ' + posfija.pop() + ' ' + var1 + ';\n'
                // else {
                //     pilaS.push("float")
                // }
            } else {
            ...
            if ((tipo1 == "bool" || boelanos.indexOf(tipo1) != -1) && (tipo2 == "bool" || boelanos.indexOf(tipo2) != -1)) {
...
function ArmarPosfija(ent) {
    ...
    if (ent == "(" || ent == "[") {
        ...
        if (comp_bool.indexOf(ent) != -1) {
            ...
            while (operacion_bool.indexOf(pila_posfija[pila_posfija.length - 1]) != -1) {
                posfija.push(pila_posfija.pop());
            }
            pila_posfija.push(ent);
        } else console.log("Error Semantico")
